...
async function seedDatabase(): Promise<void> {
  ...
  await TeamModel.updateOne(
    { _id: teams[0]._id },
    { members: [users[0]._id, users[1]._id], totalPoints: users[0].totalPoints + users[1].totalPoints },
  );
  await TeamModel.updateOne(
    { _id: teams[1]._id },
    { members: [users[2]._id, users[3]._id], totalPoints: users[2].totalPoints + users[3].totalPoints },
  );
  await TeamModel.updateOne(
    { _id: teams[2]._id },
    { members: [users[4]._id, users[5]._id], totalPoints: users[4].totalPoints + users[5].totalPoints },
  );
  ...
  const refreshedTeams = await TeamModel.find().sort({ totalPoints: -1 }).lean();
  ...
  await LeaderboardModel.insertMany([...weeklyLeaderboard, ...monthlyLeaderboard]);
